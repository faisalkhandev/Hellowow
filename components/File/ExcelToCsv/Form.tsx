  ...
  const handleConversion = async () => {
    ...
    try {
        ...
        reader.onload = (event) => {
            ...
            const workbook = XLSX.read(data, { type: "array" }); // Use 'array' type for Uint8Array
            const firstSheetName = workbook.SheetNames[0];
            const worksheet = workbook.Sheets[firstSheetName];

            // Convert the worksheet to JSON format with raw values
            const jsonData: unknown[][] = XLSX.utils.sheet_to_json(worksheet, { header: 1, raw: true }); // Get data as an array of arrays
            ...
            const csv = jsonData.map((row: unknown[]) => // Explicitly define row as unknown[]
                row.map(cell => {
                    // Handle different types of cell values
                    if (cell === null || cell === undefined) {
                        return ''; // Convert null/undefined to empty string
                    } else if (typeof cell === 'object') {
                        return JSON.stringify(cell); // Convert objects to JSON string
                    } else {
...
//             const csv = XLSX.utils.sheet_to_csv(worksheet, { header: 1 }); // Use header: 1 to get the first row as headers
