...
const ExcelSplitter = () => {
  ...
  const handleSplit = async () => {
    ...
    if (files.length === 0 || rowsPerFile <= 0) {
      ...
      setError("Please upload a valid Excel file and specify rows per file.");
      return;
    }
    ...
    reader.onload = async (e) => {
      ...
      try {
        ...
        const newFiles = chunkedData.map((chunk, index) => {
          const newSheet = utils.aoa_to_sheet(chunk);
          const newWorkbook = utils.book_new();
          utils.book_append_sheet(newWorkbook, newSheet, `Sheet1`);

          // Create a Blob for the new file
          const fileName = `split_file_${index + 1}.xlsx`;
        ...
        setError("Error processing the file. Please ensure it is a valid Excel file."); // Notify user about the error
    ...
    <>
      <div>
        ...
        {files.length > 0 && generatedFiles.length === 0 && (
          <Card className="flex flex-col space-y-4 w-full max-w-[940px] justify-center items-center h-[450px] shadow-2xl border-none mx-auto">
            <div className="flex items-center justify-center space-x-2 h-[calc(450px-100px)]">
              <div className="flex max-lg:flex-col max-lg:items-center gap-3 items-end">
              <div className="w-[250px] space-y-1">
                <Label className="text-[0.78rem] text-paragraph">
                  Rows Per File
                </Label>
                <Input
                  type="number"
              ...
              {
                error?(
                  <Button onClick={resetState} size="lg" className="h-10">
                Remove
              </Button>
                ):(
          ...
          <Card className="flex w-full max-w-[940px] justify-center items-center h-[450px] shadow-2xl border-none mx-auto">
            <div className="text-center">
              <div className="mb-6">
                ...
                <p className="text-gray-400 mt-2 text-center">
                 Your file has been splited successfully
                </p>
              </div>
