...
const Login = ({ onFormSwitch }: { onFormSwitch: (form: "signin" | "signup" | "forgotPassword") => void }) => {
  ...
  const googleClick = async () => {
    // Call signIn with the provider name
    await signIn("google", {
    ...
    <Card className='w-full m-auto rounded-lg border-none'>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-3">
            ...
            <div className='w-full flex justify-between items-center text-[0.86rem] pl-[2px] pt-2 pb-4'>
              ...
              <p className="text-primary cursor-pointer" onClick={() => onFormSwitch("forgotPassword")}>Forgot Password?</p>
            </div>
      ...
      <CardFooter className='px-4 flex flex-col'>
        <Button variant="outline" className='w-full h-12' onClick={googleClick}>
          <Image src={Google} alt="google" priority width={30} height={30} />

        </Button>
        <p className="text-center text-[0.75rem] pt-4 font-poppin text-paragraph">
          Don&apos;t have an account?
          <span className="text-primary cursor-pointer font-poppin text-[0.8rem]" onClick={() => onFormSwitch("signup")}>
            Sign Up
          </span>
        </p>
      </CardFooter>
