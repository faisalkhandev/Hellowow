...
const schema = z.object({
  ...
  purchase_method: z.string().min(2, {
    message: "Please enter at least 2 characters",
  }),
...
const EssayForm = () => {
  ...
  const contractData = [
    ...
    },
    {
      ...
      placeholder: "Cash", // Adjust if more methods are needed
    },
    {
  ...
  const onSubmit: SubmitHandler<FormFields> = async (values) => {
    try {
      const content = await generateContent("bill_of_sale_generator",{
        topic:'Bill of Sale ',
        ...
        purchaseMethod:values.purchase_method,
        otherInfo:values.important_info,
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        {
          !generatedText?(
            ...
            <div className='grid grid-cols-1 md:grid-cols-[0.6fr_1fr] gap-6 mt-8 w-full min-h-[500px]  '>
            <Card className="w-full shadow-xl border-none  space-y-6 flex-col   max-lg:px-1  pt-5  pb-0 ">
              <div className='px-5'>
            <h1 className="text-[1.35rem] font-poppin text-heading font-bold">Bill of Sale</h1>
            <p className="text-paragraph text-[0.85rem] sm:text-[0.9rem] font-poppin mb-2">Paste your info below</p>
            </div>
