...
const Form: React.FC<EssayFormProps> = ({ setGeneratedText,setTopic }) => {
  ...
  const onSubmit: SubmitHandler<FormFields> = async (values) => {
    try {
      const content = await generateContent("blog_post_rewriter",{
        topic: values.topic,
      });
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="h-full">
        <Card className="w-full border-none shadow-lg h-full">
          <CardContent className="space-y-4 flex flex-col pt-6 h-full">
            <div>
              <h1 className="text-[1.3rem] font-poppin text-heading font-bold">Blog Post</h1>
              <p className="text-paragraph text-[0.9rem] font-poppin">Rewrite your post in a second</p>
            </div>
