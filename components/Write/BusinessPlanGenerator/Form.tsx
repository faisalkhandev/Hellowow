...
const Form: React.FC<EssayFormProps> = ({ setGeneratedText,setTopic }) => {
  ...
  const onSubmit: SubmitHandler<FormFields> = async (values) => {
    try {
      const content = await generateContent("business_plan_generator",{
        topic: values.topic,
      });
    ...
    <FormProvider {...methods}>
      <form onSubmit={handleSubmit(onSubmit)} className="h-full">
        <Card className="w-full border-none shadow-lg h-full">
          <CardContent className="space-y-[0.1rem] flex flex-col pt-6 h-full">
            <div>
              <h1 className="text-[1.3rem] font-poppin text-heading font-bold">Plan Generator</h1>
              <p className="text-paragraph text-[0.9rem] font-poppin">What does your business do?</p>
            </div>
            ...
            <div className="flex-1 flex-grow">
              <TextArea
                ...
                name="topic"
                Label=""
                placeHolder='i.e.dog walking service'
              />
            </div>
