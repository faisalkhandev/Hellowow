...
import { generateContent } from '@/actions/write/action'; // Import your server action
...
const Form: React.FC<EssayFormProps> = ({ setGeneratedText, setTopic }) => {
  ...
  const onSubmit: SubmitHandler<FormFields> = async (data) => {
    try {
      const response = await generateContent("paraphrase", { topic: data.content });
        ...
        <Card className="w-full border-none shadow-lg h-full">
          <CardContent className="space-y-1 flex flex-col pt-6 h-full">
            <div>
              ...
              <p className="text-paragraph text-[0.9rem] font-poppin">Rewrite your article in seconds</p>
            </div>
