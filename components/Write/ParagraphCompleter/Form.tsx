...
const Form: React.FC<ParagraphFormProps> = ({ setGeneratedText,setTopic }) => {
  ...
  const onSubmit: SubmitHandler<FormFields> = async (values) => {
    try {
      const content = await generateContent("paragraph_completer",{
        topic: values.topic,
      });
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="h-full">
        <Card className="w-full border-none shadow-lg h-full">
          <CardContent className="space-y-6 flex flex-col pt-6 h-full">
            <div>
              <h1 className="text-[1.3rem] font-poppin text-heading font-bold">Starting sentences</h1>
              <p className="text-paragraph text-[0.9rem] font-poppin">Complete your text in a second</p>
            </div>
