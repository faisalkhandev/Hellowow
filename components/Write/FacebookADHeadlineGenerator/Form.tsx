...
const Form: React.FC<EssayFormProps> = ({ setGeneratedText,setTopic }) => {
  ...
  const onSubmit: SubmitHandler<FormFields> = async (values) => {
    try {
      const content = await generateContent("facebook_ad_headline_generator",{
        topic: values.topic,
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="h-full">
        <Card className="w-full border-none shadow-lg h-full">
          <CardContent className="space-y-6 flex flex-col pt-6 h-full">
            <div>
              <h1 className="text-[1.3rem] font-poppin text-heading font-bold">About Company</h1>
            ...
            <div className="flex-1 flex-grow">
              <TextArea
                ...
                name="from"
                Label="What it does"
                placeHolder='Free Online Tools Inluding Pdf ,Image,File Conversion,AI Writing and more
metaData:Facebook Ad Headline Generator-NifaWow'
              />
            </div>
