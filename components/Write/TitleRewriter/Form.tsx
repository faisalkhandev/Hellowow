...
const Form : React.FC<EssayFormProps> = ({ setGeneratedText,setTopic }) => {
  ...
  const onSubmit: SubmitHandler<FormFields> = async (values) => {
    try {
      const content = await generateContent("title_rewriter",{
        topic: values.topic,
        ...
        <Card className="w-full border-none shadow-lg h-full">
          <CardContent className="space-y-6 flex flex-col pt-6 h-full">
            <div>
              ...
              <p className="text-paragraph text-[0.9rem] font-poppin">Enter an article topic</p>
            </div>
            ...
            <div className="flex-1 flex-grow">
              <TextArea
                ...
                name="topic"
                Label=""
                placeHolder='i.e. how to use google'
              />
            </div>
