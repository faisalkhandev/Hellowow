...
const schema = z.object({
  article_writer: z.string().min(1, "Article title is required"),
});
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <Card className="w-full shadow-xl border-none mt-6 space-y-6  flex-col text-center justify-center items-center max-lg:px-4 min-h-[332px] py-8">
        ...
        <CardContent className="w-full  flex flex-col justify-center space-y-4 items-center">
          <Input
            ...
            ${errors.article_writer ? 'focus:border-[#F6CCD0] border-[#DC3545]' : 'focus:border-[#BFDEFF]'} focus:outline-none rounded-lg w-full`}
          />
          {errors.article_writer && (
            <p className="text-red-600 text-sm">
              {errors.article_writer.message}
            </p>
          )}
          <Button
            type="submit"
