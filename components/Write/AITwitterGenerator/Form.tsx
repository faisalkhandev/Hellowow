...
const Form : React.FC<EssayFormProps> = ({ setGeneratedText,setTopic }) => {
  ...
  const onSubmit: SubmitHandler<FormFields> = async (values) => {
    try {
      const content = await generateContent("twitter_post_generator",{
        topic: values.topic,
        tweetCount:values.count_tweets,
  ...
  const Count_Tweets = [
    { label: "Single Tweet", value: "single_tweet" },
