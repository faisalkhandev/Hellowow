...
const Form: React.FC<EssayFormProps> = ({ setGeneratedText,setTopic }) => {
  ...
  const onSubmit: SubmitHandler<FormFields> = async (values) => {
    try {
      const content = await generateContent("slogan_generator",{
        topic: values.topic,
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="h-full">
        <Card className="w-full border-none shadow-lg h-full">
          <CardContent className="space-y-6 flex flex-col pt-6 h-full">
            <div>
              <h1 className="text-[1.3rem] font-poppin text-heading font-bold">Slogan Generator</h1>
              <p className="text-paragraph text-[0.9rem] font-poppin">What&apos;s your business do?</p>
            </div>
            <InputField name="business_name" Label="Business Name"  length={50}/>
            ...
            <div className="flex-1 flex-grow">
              <TextArea
                ...
                name="topic"
                Label="What it Does"
                placeHolder='i.e.marketing agency dog for influencer'
              />
            </div>
