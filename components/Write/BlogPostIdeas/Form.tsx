...
const schema = z.object({
  ...
  target_audience: z.string().min(5, { message: "Topic must be at least 5 characters" }),
...
const Form : React.FC<EssayFormProps> = ({ setGeneratedText,setTopic }) => {
  ...
  const { handleSubmit,formState:{isSubmitting} } = methods;
    const onSubmit: SubmitHandler<FormFields> = async (values) => {
      try {
        const content = await generateContent("blog_post_ideas",{
          topic: values.topic,
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="h-full">
        <Card className="w-full border-none shadow-lg h-full">
          <CardContent className="space-y-6 flex flex-col pt-6 h-full">
            <div>
              <h1 className="text-[1.3rem] font-poppin text-heading font-bold">About Blog</h1>
              <p className="text-paragraph text-[0.9rem] font-poppin">Tell us about your blog</p>
            </div>
