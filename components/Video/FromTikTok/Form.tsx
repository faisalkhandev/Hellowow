...
const schema = z.object({
  tiktok_url: z
    ...
    .refine((url) => url.includes("tiktok.com"), {
      message: "URL must be a TikTok URL",
    }), // Custom validation for TikTok URL
});
...
const Form = () => {
  ...
  const onSubmit: SubmitHandler<FormFields> = async (data) => {
    ...
    try {
      const rapidapi=process.env.NEXT_PUBLIC_RAPID_API;
      ...
      const response = await fetch('https://social-download-all-in-one.p.rapidapi.com/v1/social/autolink', {
        method: 'POST',
        headers: {
          'X-RapidAPI-Key': rapidapi, // Use the defined RapidAPI key
          'X-RapidAPI-Host': 'social-download-all-in-one.p.rapidapi.com',
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ url: data.tiktok_url }), // Pass the URL to the API
      });
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = title ? `${title}.mp4` : 'video.mp4'; // Use title or default name
            ...
            <CardContent className="space-y-6 flex justify-center items-center h-full min-h-[400px]">
              <div className='flex flex-col items-start gap-2'>
                <div className='w-[250px]'>
                  <InputField name="tiktok_url" Label="TikTok URL" />
                </div>
