...
import { ConvertVideoFile, MuteVideoFile } from '@/utils/convert'; // Import the conversion function
...
const VideoCompressor = () => {
  ...
  const [videoSettings] = useState<videoSettingsInput>({
    ...
    videoType: VideoFormat.MP3, // Set to MP3 for conversion
    customEndTime: 0,
    customStartTime: 0,
    removeAudio: false,
  });
console.log(videoSettings,"videoSetting");
  ...
  const condense = async () => {
    ...
    try {
      ...
      const { url, output, outputBlob } = await MuteVideoFile(
        ffmpegRef.current,
        video,
        videoSettings
      );
              ...
              <div className="block mt-4">
                ...
                <div>
                  <div className="flex justify-between space-x-2 mt-2">
                    ...
                    >
                      {disableDuringCompression ? "Converting..." : "Convert to MP3"}
                    </Button>
                  </div>
                </div>
              </div>
