...
import {compressFile} from '@/utils/convert';
...
const VideoCompressor = () => {
  ...
  const condense = async () => {
    ...
    try {
      ...
      const { url, output, outputBlob } = await compressFile(
        ffmpegRef.current,
        video,
        videoSettings
      );
      ...
      // Set compressed video information
      setCompressedVideoInfo({
        url,
        name: output,
        size: sizeInMB,
        type: video.fileType.split("/")[0], // Get the type from the original file
      });
  ...
  return (
    <>
      {status === "converted" && compressedVideoInfo ? (
        <div className="mt-4">
          <h3 className="text-lg font-bold">Compressed Video Information</h3>
          <VideoDisplay videoUrl={compressedVideoInfo.url} />
              ...
              <div className="block mt-4">
                ...
                <div>
                  <div className="flex justify-between space-x-2 mt-2">
                    ...
                    >
                      {disableDuringCompression ? "Converting..." : "Compress"}
                    </Button>
                  </div>
                </div>
              </div>
