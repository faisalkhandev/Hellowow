...
import { IntelliHubTools } from '@/constants/Intellihub/intelihubTools'
import Link from 'next/link'
import React from 'react'

const IntelliHubToolPage = async ({ searchParams }: { searchParams: { search?: string } }) => {
  const searchQuery = searchParams.search || "";
  const filteredTools = IntelliHubTools.filter((tool) => {
    return (
  ...
  return (
    ...
    <div className="padding-x py-2 lg:py-4 ">
           <HeroSection paragraph="Free Online Tools"  title="Intellihub Tools"/>
         </div>
...
export default IntelliHubToolPage