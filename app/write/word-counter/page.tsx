...
import Form from '@/components/Write/WordCounter/Form'
import Rating from '@/components/Write/YoutubeScriptWriter/Rating'
import { Metadata } from 'next'


export const metadata: Metadata = {
  title: "Word Count Tool - NifaWow",
  description: "Quickly count the words in your text with our easy-to-use word count tool. Paste your text and get instant feedback on word, character, and sentence count.",
};
    ...
    <>
     <div className="w-full flex flex-col justify-center items-center py-16  space-y-8 padding-x">
      <div className='text-center'>
        <h1 className="text-[1.6rem] sm:text-[40px] font-bold  font-poppin text-heading  dark:text-white ">Word Counter</h1>
        <p className='text-paragraph font-poppin  text-[1rem]'>
        Count words in your text</p>
        </div>
        <div className='grid grid-cols-1 max-container  gap-6 pt-8 w-full  md:px-24 '>
          <Form />
          </div>
        ...
        <div className="max-container">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 mt-8  ">
            {aiWritingToolsArray
              .filter(card => card.title !== 'Word Counter') 
              .slice(0, 8) 
              .map((card, index) => (
                <MainToolCard
                  key={index}
