...
import Rating from '@/components/Write/TitleRewriter/Rating'
import QrCode from '@/components/Write/QrCode'
import { Metadata } from 'next'
import TitleRewriterClient from '@/components/Write/TitleRewriter/TitleRewriterClient'



export const metadata: Metadata = {
  title: "Title Rewriter - Effortlessly Generate New Titles - NifaWow",
  description: "Rewrite your article titles to improve engagement and creativity. Enter your title, and let AI generate fresh, catchy title ideas for you.",
};
    ...
    <>
     <div className="w-full flex flex-col justify-center items-center py-16  space-y-8 padding-x">
      <div className='text-center'>
        <h1 className="text-[1.6rem] sm:text-[40px] font-bold  font-poppin text-heading  dark:text-white ">Title Rewriter</h1>
        <p className='text-paragraph font-poppin  text-[1rem]'>
        Rewrite your article title</p>
        </div>
        ...
        <div className="max-container">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 mt-8  ">
            {aiWritingToolsArray
              .filter(card => card.title !== 'Title Rewriter') 
              .slice(0, 8) 
              .map((card, index) => (
                <MainToolCard
                  key={index}
