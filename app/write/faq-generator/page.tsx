...
import Rating from '@/components/Write/FAQGenerator/Rating'
import QrCode from '@/components/Write/QrCode'
import { Metadata } from 'next'
import FAQGeneratorClient from '@/components/Write/FAQGenerator/FAQGeneratorClient'


export const metadata: Metadata = {
  title: "FAQ Generator - Tell Us Your Title, We'll Suggest FAQs - NifaWow",
  description: "Easily generate frequently asked questions for your article. Provide your title and let AI suggest relevant FAQs for your audience.",
};
    ...
    <>
     <div className="w-full flex flex-col justify-center items-center py-16  space-y-8 padding-x">
      <div className='text-center'>
        <h1 className="text-[1.6rem] sm:text-[40px] font-bold  font-poppin text-heading  dark:text-white ">FAQ Generator</h1>
        <p className='text-paragraph font-poppin  text-[1rem]'>
        Generate FAQs easily for your article</p>
        </div>
        ...
        <div className="max-container">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 mt-8  ">
            {aiWritingToolsArray
              .filter(card => card.title !== 'FAQ Generator') 
              .slice(0, 8) 
              .map((card, index) => (
                <MainToolCard
                  key={index}
