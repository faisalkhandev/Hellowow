...
import Rating from '@/components/Write/AIPollGenerator/Rating'
import QrCode from '@/components/Write/QrCode'
import { Metadata } from 'next'
import AIPollGeneratorClient from '@/components/Write/AIPollGenerator/AIPollGenrator'


export const metadata: Metadata = {
  title: "Poll Generator - NifaWow",
  description: "Generate interactive polls effortlessly with AI. Enter a topic, and we'll provide a question along with answer options for your audience.",
};
    ...
    <>
     <div className="w-full flex flex-col justify-center items-center py-16  space-y-8 padding-x">
      <div className='text-center'>
        ...
        Poll Generator</h1>
        <p className='text-paragraph font-poppin  text-[1rem]'>Let AI Generate a Poll</p>
        </div>
        <AIPollGeneratorClient/>
        <QrCode />
        ...
        <div className="max-container">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 mt-8  ">
            {aiWritingToolsArray
              .filter(card => card.title !== 'Poll Generator') 
              .slice(0, 8) 
              .map((card, index) => (
                <MainToolCard
                  key={index}
