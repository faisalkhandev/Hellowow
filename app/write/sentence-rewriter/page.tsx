...
import Rating from '@/components/Write/SentenceRewriter/Rating'
import QrCode from '@/components/Write/QrCode'
import { Metadata } from 'next'
import SentenceRewriterClient from '@/components/Write/SentenceRewriter/SentenceRewriterClient'


export const metadata: Metadata = {
  title: "AI Sentence Rewriter - Unique Sentence Rewriting - NifaWow",
  description: "Enter your sentence and let our AI rewrite it uniquely in seconds. Perfect for improving your writing or generating new ideas.",
};
    ...
    <>
     <div className="w-full flex flex-col justify-center items-center py-16  space-y-8 padding-x">
      <div className='text-center'>
        <h1 className="text-[1.6rem] sm:text-[40px] font-bold  font-poppin text-heading  dark:text-white ">Sentence Rewriter</h1>
        <p className='text-paragraph font-poppin  text-[1rem]'> Enter your sentence(s) and let us rewrite them uniquely</p>
        </div>
        <SentenceRewriterClient/>
        {/* <div className='grid max-container grid-cols-1 md:grid-cols-[.47fr_1fr] gap-6 mt-8 w-full min-h-[624px] md:px-24'>
          <Form />
        ...
        <div className="max-container">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 mt-8  ">
            {aiWritingToolsArray
              .filter(card => card.title !== 'Sentence Rewriter') 
              .slice(0, 8) 
              .map((card, index) => (
                <MainToolCard
                  key={index}
