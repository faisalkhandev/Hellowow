...
import Rating from '@/components/Write/BusinessNameGenerator/Rating'
import QrCode from '@/components/Write/QrCode'
import { Metadata } from 'next'
import BusinessNameGeneartorClient from '@/components/Write/BusinessNameGenerator/BusinessNameGeneratorclient'


export const metadata: Metadata = {
  title: "Free Business Name Generator - Generate Ideas Quickly & Easily - NifaWow",
  description: "Generate unique business names quickly with AI. Enter your business idea, and get creative name suggestions tailored to your needs.",
};
    ...
    <>
     <div className="w-full flex flex-col justify-center items-center py-16  space-y-8 padding-x">
      <div className='text-center'>
        <h1 className="text-[1.6rem] sm:text-[40px] font-bold  font-poppin text-heading  dark:text-white ">Business Name Generator</h1>
        <p className='text-paragraph font-poppin  text-[1rem]'>
        Generate a business name for your idea</p>
        </div>
        ...
        <div className="max-container">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 mt-8  ">
            {aiWritingToolsArray
              .filter(card => card.title !== 'Business Name Generator') 
              .slice(0, 8) 
              .map((card, index) => (
                <MainToolCard
                  key={index}
