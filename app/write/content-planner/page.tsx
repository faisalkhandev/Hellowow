...
import Rating from '@/components/Write/Contentplanner/Rating'
import QrCode from '@/components/Write/QrCode'
import { Metadata } from 'next'
import ContentPlannerClient from '@/components/Write/Contentplanner/ContentPlannerClient'

export const metadata: Metadata = {
  title: "Free AI Content Planner - Map Out a Content Strategy - NifaWow",
  description: "Generate a simple content plan with AI. Let us help you map out a content strategy for your topic with ease.",
};
    ...
    <>
     <div className="w-full flex flex-col justify-center items-center py-16  space-y-8 padding-x">
      <div className="text-center">
        <h1 className="text-[1.6rem] sm:text-[40px] font-bold  font-poppin text-heading  dark:text-white ">Content Planner</h1>
        <p className='text-paragraph font-poppin  text-[1rem]'>
        Generate a simple content plan</p>
        </div>
        ...
        <div className="max-container">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 mt-8  ">
            {aiWritingToolsArray
              .filter(card => card.title !== 'Content Planner') 
              .slice(0, 8) 
              .map((card, index) => (
                <MainToolCard
                  key={index}
