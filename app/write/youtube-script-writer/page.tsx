...
import Rating from '@/components/Write/YoutubeScriptWriter/Rating'
import QrCode from '@/components/Write/QrCode'
import { Metadata } from 'next'
import YoutubeScriptWriterClient from '@/components/Write/YoutubeScriptWriter/YoutubeScriptWriterClient'
export const metadata: Metadata = {
  title: "YouTube Script Writer - AI-Powered Script Generator - NifaWow",
  description: "Generate YouTube video scripts with AI. Perfect for content creators looking for quick and effective scripts.",
};
    ...
    <>
     <div className="w-full flex flex-col justify-center items-center py-16  space-y-8 padding-x">
      <div className="text-center">
        <h1 className="text-[1.6rem] sm:text-[40px] font-bold  font-poppin text-heading  dark:text-white ">YouTube Script Writer</h1>
        <p className='text-paragraph font-poppin  text-[1rem]'>
        Easily generate a YouTube video script</p>
        </div>
        ...
        <div className="max-container">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 mt-8  ">
            {aiWritingToolsArray
              .filter(card => card.title !== 'YouTube Script Writer') 
              .slice(0, 8) 
              .map((card, index) => (
                <MainToolCard
                  key={index}
