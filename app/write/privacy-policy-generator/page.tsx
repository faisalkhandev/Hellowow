...
import Rating from '@/components/Write/PrivacyPolicyGenerator/Rating'
import QrCode from '@/components/Write/QrCode'
import { Metadata } from 'next'
import PrivacyPolicyGeneratorClient from '@/components/Write/PrivacyPolicyGenerator/PrivacyPolicyGenerator'

export const metadata: Metadata = {
  title: "Privacy Policy Generator - NifaWow",
  description: "Create a comprehensive privacy policy for your business in seconds. Enter your company name, address, and website URL to generate a tailored policy.",
};
    ...
    <>
     <div className="w-full flex flex-col justify-center items-center py-16  space-y-8 padding-x">
      <div className='text-center'>
        <h1 className="text-[1.6rem] sm:text-[40px] font-bold  font-poppin text-heading  dark:text-white ">Privacy Policy Generator</h1>
        <p className='text-paragraph font-poppin  text-[1rem]'>
        Generate privacy policy for your</p>
        </div>
        <PrivacyPolicyGeneratorClient/>
        <QrCode />
        ...
        <div className="max-container">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 mt-8  ">
            {aiWritingToolsArray
              .filter(card => card.title !== 'Privacy Policy Generator') 
              .slice(0, 8) 
              .map((card, index) => (
                <MainToolCard
                  key={index}
