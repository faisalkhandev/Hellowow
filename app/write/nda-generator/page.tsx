...
import Rating from '@/components/Write/NDAGenerator/Rating'
import QrCode from '@/components/Write/QrCode'
import { Metadata } from 'next'
import NDAGeneratorClient from '@/components/Write/NDAGenerator/NDAGenratorClient'


export const metadata: Metadata = {
  title: "NDA Generator - NifaWow",
  description: "Generate a Non-Disclosure Agreement (NDA) quickly and easily. Specify the disclosing party and receiving party to create a customized NDA in seconds.",
};
    ...
    <>
     <div className="w-full flex flex-col justify-center items-center py-16  space-y-8 padding-x">
      <div className='text-center'>
        <h1 className="text-[1.6rem] sm:text-[40px] font-bold  font-poppin text-heading  dark:text-white ">NDA Generator</h1>
        <p className='text-paragraph font-poppin  text-[1rem] font-[400]'>
        Generate NDA for your</p>
        </div>
        ...
        <div className="max-container">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 mt-8  ">
            {aiWritingToolsArray
              .filter(card => card.title !== 'NDA Generator') 
              .slice(0, 8) 
              .map((card, index) => (
                <MainToolCard
                  key={index}
