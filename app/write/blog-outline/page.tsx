...
import BlogOutlineClient from '@/components/Write/BlogOutline/BlogOutlineClient'

export const metadata: Metadata = {
  title: "Blog Outline Generator - NifaWow",
  description: "Generate structured blog outlines effortlessly. Enter your title, and let AI create a detailed outline for your blog post.",
};
    ...
    <>
     <div className="w-full flex flex-col justify-center items-center py-16  space-y-8 padding-x">
      <div className="text-center">
        <h1 className="text-[1.6rem] sm:text-[40px] font-bold  font-poppin text-heading  dark:text-white ">Blog Outline Generator</h1>
        <p className='text-paragraph font-poppin  text-[1rem]'>
        Write Blog Outline</p>
        </div>
        <BlogOutlineClient/>
        <QrCode />
        ...
        <div className="max-container">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 mt-8  ">
            {aiWritingToolsArray
              .filter(card => card.title !== 'Blog Outline Generator') 
              .slice(0, 8) 
              .map((card, index) => (
                <MainToolCard
                  key={index}
