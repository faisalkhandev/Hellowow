...
import Rating from '@/components/Write/LinkdinPostGenerator/Rating'
import QrCode from '@/components/Write/QrCode'
import { Metadata } from 'next'
import LinkdinPostGeneratorClient from '@/components/Write/LinkdinPostGenerator/LinkdinPostGeneratorClient'

export const metadata: Metadata = {
  title: "LinkedIn Post Generator - Professional AI Writing - NifaWow",
  description: "Create professional LinkedIn posts instantly with our AI-powered generator. Free and easy to use.",
};



const LinkdInPostGenerator = () => {
  return (
    <>
     <div className="w-full flex flex-col justify-center items-center py-16  space-y-8 padding-x">
      <div className='text-center'>
        <h1 className="text-[1.6rem] sm:text-[40px] font-bold  font-poppin text-heading  dark:text-white ">LinkedIn Post Generator</h1>
        <p className='text-paragraph font-poppin  text-[1rem]'>Generate linkedin post for your</p>
        </div>
        ...
        <div className="max-container">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 mt-8  ">
            {aiWritingToolsArray
              .filter(card => card.title !== 'LinkedIn Post Generator') 
              .slice(0, 8) 
              .map((card, index) => (
                <MainToolCard
                  key={index}
