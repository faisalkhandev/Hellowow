...
const PdfPage = async ({ searchParams }: { searchParams: { search?: string } }) => {
  const searchQuery = searchParams.search || "";
  const filteredTools = pdfPageTools.filter((tool) => {
    return (
  ...
  return (
    <div>
      <div className="padding-x py-2 lg:py-4 ">
        <HeroSection paragraph="Free Online PDF Tools" title="Pdf Tools" />
      </div>
