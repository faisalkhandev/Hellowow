...
async function convertTiff(fileBuffer: Buffer, format: string,fileName:string) {
...
export const convertImageAction =async(formData: FormData) => {
  ...
  try {
    ...
    const { processedBuffer, outputFileName } = await convertTiff(fileBuffer, format,fileName);
