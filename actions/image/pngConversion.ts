...
async function convertPng(fileBuffer: Buffer, format: string,fileName:string) {
...
export const convertImageAction =async(formData: FormData) => {
  ...
  try {
    ...
    const { processedBuffer, outputFileName } = await convertPng(fileBuffer, format,fileName);
